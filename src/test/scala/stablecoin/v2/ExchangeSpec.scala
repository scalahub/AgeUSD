  ...
  property("One complete exchange") {
    ergoClient.execute { implicit ctx: BlockchainContext =>
      ...
      val (secondBankBox, secondReceiptBox) = getOutBoxes(
        ...
        rcCircDelta = 0,
        StableCoin.getDeltaBcForDeltaSC(firstBankBox.value - minStorageRent, firstBankBox.registers(0).asInstanceOf[KioskLong].value, scMintDelta, ergToUsdEpoch2),
        Some(scToken)
      )
      ...
      val (fourthBankBox, fourthReceiptBox) = getOutBoxes(
        ...
        rcCircDelta = 0,
        StableCoin.getDeltaBcForDeltaSC(thirdBankBox.value - minStorageRent, thirdBankBox.registers(0).asInstanceOf[KioskLong].value, scRedeemDelta, ergToUsdEpoch4),
        None
      )
