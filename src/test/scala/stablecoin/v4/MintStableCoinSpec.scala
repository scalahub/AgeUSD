...
class MintStableCoinSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with HttpClientTesting {
  ...
  property("Mock Mint StableCoin") {
    ergoClient.execute { implicit ctx: BlockchainContext =>
      ...
      // Mint SC
      /////////////////////////////////////////////////
      val nanoErgsPerUsd = 2864147744L
      val scMintDelta = 100L

      val scTokensIn = 9999999999998L
      val rcTokensIn = 9999999969020L
      val bankNanoErgsIn = 33398283021L
      ...
      val bcReserveIn = (bankNanoErgsIn - minStorageRent).max(0)

      val bcDelta = StableCoin.getDeltaBcForDeltaSC(
        bcReserveIn,
        scCircIn,
        scMintDelta,
        nanoErgsPerUsd / 100
      ...
      println(s"rcMintDelta $scMintDelta")
